...
const header = () => {
    return `
        <header id='header'>
            <span id='caption'>New York Times Best Sellers</span>
...
`
const bookCard = (num, title, sub, text) => {
    return`
        <div class='card'>
            <p class='numP'>${num}</p>
            <p class='sub'>${sub}</p>
            ...
            <p class='text'>${text}</p>
            <button><span class='details'>read more</span><span class="material-symbols-sharp" id='arrowIcon'>
            arrow_forward</span></button>
        </div>
    `
}

const makeBookCards = (obj) => {
    return obj.cards.map((x,i) => bookCard(i+1, x.title, x.sub, x.text)).join(' ')
}
...
const loadEvent = () => {
    ...
    rootElement.insertAdjacentHTML('beforeend', bookCardsDiv)

    const bookCardsDivElement = document.getElementById('bookCards')
    bookCardsDivElement.insertAdjacentHTML('beforeend', makeBookCards(books))
